...
| NOW — BROKER-ADOPT | Register and maintain an adopter's own .NET application | Existing Broker/SDK protection and sample adoption are integrated; local candidate tooling and the distinct evaluation app are implemented on the branch. | **Candidate PASS on branch `4a8eb70...`; pending focused review/integration.** Supported administrator tooling for explicit registration, inspection, executable update and revocation; no manual service-configuration edits. One ordinary-account real-service proof covers use, restart, old-ciphertext preservation and denial after revocation. |
